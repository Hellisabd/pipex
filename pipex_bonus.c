...
/*   Updated: 2024/01/31 17:08:11 by bgrosjea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cmd(t_pipex *p, char **argv, char **env)
{
	...
	while (p->cmd[i])
	{
		...
		if (!p->cmd[i])
			exit ((ft_free_tab(p->cmd), close_fd(p), \
			ft_free_tab(p->path), 1));
		p->cmd[i] = ft_strtrim(p->cmd[i], " ");
		if (!p->cmd[i])
			exit ((ft_free_tab(p->cmd), close_fd(p), \
			ft_free_tab(p->path), 1));
...
void	do_pipe(t_pipex *p, char **argv, char **env)
{
	...
	p->pid = fork();
	if (p->pid < 0)
		exit ((close_fd(p), ft_free_tab(p->path), perror("Error"), 1));
	if (p->pid == 0)
	{
		if (dup2(p->fd[1], 1) < 0)
			exit ((close_fd(p), \
			ft_free_tab(p->path), perror("Error"), 1));
		close_fd(p);
		cmd(p, argv, env);
	}
	else
